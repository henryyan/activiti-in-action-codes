				...
				<tr>
					...
					<td>
						<c:if test="${empty leave.task.assignee }">
							<a class="btn" href="claim/${leave.task.id}"><i class="glyphicon glyphicon-eye-open"></i>签收</a>
						</c:if>
						<c:if test="${not empty leave.task.assignee }">
							<a class="btn" href="view/${leave.task.id}"><i class="glyphicon glyphicon-user"></i>办理</a>
						</c:if>
					</td>
				</tr>
			</c:forEach>
